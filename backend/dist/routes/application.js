...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        const getString = (f) => (Array.isArray(f) ? String(f[0]) : f ? String(f) : undefined);
        const toFile = (f) => (Array.isArray(f) ? f[0] : f);
        ...
        if (existing)
            return res.status(400).json({ success: false, error: 'You already have an active or pending rental application.' });
